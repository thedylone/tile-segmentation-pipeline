...
from py3dtiles.typing import RefineType, TileDictType, ContentType
...
class Tile(RootProperty[TileDictType]):
    def __init__(
        ...
        contents: list[ContentType] | None = None,
    ) -> None:
        super().__init__()
        ...
        if content and contents:
            raise InvalidTilesetError(
                'The tile can\'t have both "content" and "contents"'
            )
        self.tile_content: TileContent | TileSet | None = None
        self.content: ContentType | None = content
        self.contents: list[ContentType] | None = contents
    ...
    def from_dict(cls, tile_dict: TileDictType) -> Tile:
        if "box" in tile_dict["boundingVolume"]:
            bounding_volume: BoundingVolumeBox = BoundingVolumeBox.from_dict(tile_dict["boundingVolume"])  # type: ignore [arg-type]
        elif (
        ...
        if "content" in tile_dict and "contents" in tile_dict:
            raise InvalidTilesetError(
                'The tile can\'t have both "content" and "contents"'
            )
        if "content" in tile_dict:
            tile.content = tile_dict["content"]
        if "contents" in tile_dict:
            tile.contents = tile_dict["contents"]
    ...
    def has_content(self) -> bool:
        ...
        """
        return bool(
            self.tile_content is not None or self.content or self.contents
        )
    ...
    def delete_on_disk(
        self, root_uri: Path, delete_sub_tileset: bool = False
    ) -> None:
        """
        ...
        def _delete_content(content_uri: Path | None) -> None:
            if content_uri is None:
                return
            if content_uri.is_absolute():
                tile_content_path = content_uri
            else:
                tile_content_path = root_uri / content_uri
        ...
        if self.content is not None:
            _delete_content(Path(self.content["uri"]))
        for content in self.contents or []:
            _delete_content(Path(content["uri"]))
    ...
    def add_child(self, tile: Tile) -> None:
        ...
        if tile.bounding_volume is not None:
            if self.bounding_volume is None:
                ...
                transformed_bounding_volume = copy.deepcopy(
    ...
    def write_content(self, root_uri: Path | None) -> None:
        ...
        if self.content is None:
            raise TilerException(
                "tile.content_uri is None, cannot write tile content"
            )

        if Path(self.content["uri"]).is_absolute():
            content_path = Path(self.content["uri"])
        else:
            if root_uri is None:
                raise ValueError(
                    "No root_uri given and tile.content_uri is not absolute"
                )
            content_path = root_uri / Path(self.content["uri"])
    ...
    def to_dict(self) -> TileDictType:
        ...
        if (
            ...
            and self.transform is not DEFAULT_TRANSFORMATION
        ...
        if self.content:
            dict_data["content"] = self.content

        if self.contents:
            dict_data["contents"] = self.contents
        return dict_data
